...
fn parse_log(buf: Vec<u8>) -> Session {
    ...
    loop {
        let op_type = buf[i] as char;
        i = i + 1;

        match op_type {
            'S' => {
                let _txnid = bytes2long(&buf[i..i + 8]);
                i = i + 8;
            ...
            'R' => {
                let _w_txnid = bytes2long(&buf[i..i + 8]);
                i = i + 8;
...
fn main() {
    ...
    if args.len() != 3 {
        println!("Usage: translator <Input-log-folder> <Output-log-folder>");
        return;
    }
    let str_src = &args[1];
    let str_dst = &args[2];
    ...
    for entry in fs::read_dir(src).unwrap() {
        ...
        if f_path.ends_with(".log") {
            sessions.push(get_one_session(f_path));
        }
    }
    ...
    let h = History {
        params: HistParams {
            ...
            n_event: 0, // number of events per txn
        },
        info: "dbcop logs".to_string(),
        start: start_time,
        end: end_time,
        data: sessions,
    };
    ...
    let file = File::create(dst.join(format!("history.bincode"))).expect(
        "couldn't create bincode file"
    );
    let buf_writer = BufWriter::new(file);
    bincode::serialize_into(buf_writer, &h).expect("dumping history to bincode file went wrong");
}
